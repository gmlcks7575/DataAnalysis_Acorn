...
X_train = train[['Study time']] #matrix 왜냐하면 sklearn의 LinearRegression 사용 
...
model.fit(X_train, y_train) # 모델학습은 train data를 사용 : 과적합 방지
...
print('결정계수:',r2) # 0.66321792

# r-square 함수 사용
from sklearn.metrics import r2_score
print('결정계수:',r2_score(y_test, y_pred)) # 결정계수: 0.66321792 test data를 사용

# R2 값은 위에서 언급한 바와 같이 분산을 기반으로 측정하는 도구이므로 
# 중심극한정리에 의해서 표본 데이터가 많을수록 그 정확도 역시도 올라가게된다. 
...
# 이미지와 같이 train data 수가 작을때는 오차가 너무 커서 음수가 나오다가도 
# 데이터가 어느정도 이상이 되면 괜찮은 R제곱 값을 찾아가는 것을 확인 할 수 있다.