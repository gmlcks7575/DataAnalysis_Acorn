# 선형회귀 모델을 다항회귀로 변환
# 데이터가 선형가정이 어긋날 때 (정규성 불만족) 대체할 수 있는 방법으로 다항식 항을 추가한 다항회귀 모델을 사용 가능
...
# 비선형 데이터의 경우에는 선형회귀모델로는 설명이 불완전함. 그래서 좀 더 복잡한 모델이 필요
...
model2 = LinearRegression().fit(x2, y) # 특징 행렬 값으로 학습 
