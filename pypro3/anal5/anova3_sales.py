# data.go.kr 사이트에서 어느 음식점 매출 데이터와 날씨 데이터를
# 이용하여 강수 여부에 따른 매출의 평균에 차이를 분석

# 온도가 더올 때, 보통일 때, 낮을 때의 매출액
...
# 등분산성
print(stats.levene(x1,x2,x3)) #pvalue=0.03900 < 0.05 만족X

# 정규성
print(stats.ks_2samp(x1, x2).pvalue)
print(stats.ks_2samp(x1, x3).pvalue)
print(stats.ks_2samp(x3, x2).pvalue) # 만족X
