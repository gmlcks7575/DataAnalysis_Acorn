...
print(data.head(3))
print(data.tail(3)) 
print(data['education'].unique()) #[1,2,3] #교육수준(독립) - 범주형
print(data['smoking'].unique())   #[1,2,3] #흡연율(종속) - 범주형

# 귀무 : 교육수준과 흡연률은 관련이 없다. (독립이다)
# 대립 : 교육수준과 흡연률은 관련이 있다. (독립이 아니다)

# 교차표 사용
ctab = pd.crosstab(index= data['education'], columns = data['smoking'])
#ctab = pd.crosstab(index = data['education'], columns = ['smoking'], normalize= True) #비율
...
# 대립 : 국가전체와 지역에 대한 인종 간 인원수는 관련이 있다. (독립적이지않다)
