...
# 연습1) 어느 남성 집단의 평균 키는 177이다. 남성 집단의 표본 데이터를 추출해 평균차이 검정(모집단의 평균키 177)
...
print(np.array(one_sample).mean()) # 176.219999 vs 177 평균에 차이가 있는가?
# 데이터의 정규성 확인
print(stats.shapiro(one_sample)) # pvalue=0.540051 > 0.05이므로 정규성 만족

# 검정 수행
result = stats.ttest_1samp(one_sample, popmean = 177) #t-test one sample
#popmean = 모집단의 평균 alternative = 양측, 단측
...
# A중학교 1학년 1반 학생들의 국어 시험결과는 늘 평균이 80이라고 알려져 있다.
...
print(np.mean(fdata.weight)) # 여아 몸무게 평균 3132.4444 # 3132 vs 2800 차이?
...
# 시각화
sns.displot(fdata.iloc[:, 2], kde = True) #kde 밀도그래프
plt.show()
...
# pvalue=0.0392 < 0.05 이므로 귀무가설 기각하고 대립가설을 채택