...
# 집단 간 분산이 집단 내 분산보다 충분히 큰 것인가를 파악하는 것이다.
...
from statsmodels.formula.api import ols #선형회귀모델
...
# 등분산성을 만족하지 않으면 데이터 가공을 생각할 수 있다. - 표준화, 정규화, transformation(자연log를 씌움)
...
score2 = m2['score']
score3 = m3['score']
print('등분산성 확인:', stats.levene(score1, score2, score3).pvalue) #0.113228506 > 0.05 등분산성 만족
...
print(table) #분산 분석표
# p-value : 0.939639 > 0.05 이므로 귀무 채택. 세 가지 교육방법에 따른 교육의 실기시험의 평균에 차이가 없다.
import numpy as np
