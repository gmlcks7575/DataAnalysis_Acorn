...
print('교차분석(이원카이제곱검정): 각 부서: 범주형(독립), 직원평가점수 : 범주형(종속) 간의 관련성 분석')
...
ctab = pd.crosstab(buser, rating) # 이원카이제곱은 교차표가 필요하다.
print(ctab)
...
# 귀무(영가설, H0) : 각 부서 간 연봉평균은 차이가 없다.
# 대립(연구, H1) : 각 부서 간 연봉평균은 차이가 있다.
...
print('평균:',np.mean(buser10),' ',np.mean(buser20)) #평균: 5414.285714285715   4908.333333333333
t_result = stats.ttest_ind(buser10, buser20)
print(t_result) 
#Ttest_indResult(statistic=0.4585177708256519, pvalue=0.6523879191675446)
# 해석 : pvalue=0.652 > 0.05 귀무가설 채택,  각 부서 간 연봉평균은 차이가 없다.

print('분산분석(ANOVA): 각 부서(요인 1개:부서, 4그룹이 존재)와 평균 연봉(연속형:종속) 간의 차이 분석----')
...
print('f value:', f_sta)
print('p value:', pv) # p value: 0.7454421884076983 > 0.05 이므로 귀무 채택.
