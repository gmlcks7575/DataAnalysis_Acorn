...
# 범주형 자료를 대상으로 교차빈도에 대한 기술통계량(검정통계랑)을 제공해 줄 뿐 아니라 교차빈도에 대한 
...
# 두 개의 확률변수는 범주형
# 귀무가설(H0) : 벼락치기 공부하는 것, 합격 여부는 관계가 없다.
# 대립가설(H1) : 벼락치기 공부하는 것, 합격 여부는 관계가 있다.
...
# 카이제곱표를 통해 임계치를 얻기 : df:1, 알파값:0.05 이므로  임계치:3.84
# 결론 : chi2 값 3.0 < 임계치 값 3.84 이므로 chi2값은 귀무 채택역 내에 있어 귀무가설을 채택한다. 
