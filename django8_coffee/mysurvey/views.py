...
def dataAnalysis(request):
    ...
    if pv > 0.05:
        result = "p값이 {0} > 0.05 이므로 성별과 커피브랜드 선호도는 관련이 없다.(귀무채택)".format(pv)
    else :
        result = "p값이 {0} < 0.05 이므로 성별과 커피브랜드 선호도는 관련이 있다.(귀무기각)".format(pv) 
    count = len(df)
    
    # 시각화 : 세로 막대
    # dummy 변수 생성
    df['co_num'] = df['co_survey'].apply(lambda c:1 if c== 'star' else 2
                                          if c == 'bean' else 3 if c=='ediya' else 4)
    print(df)
    
    fig = plt.gcf()
    gender_group = df['co_survey'].groupby(df['co_num']).count()
    gender_group.index = ['스타벅스','커피빈','이디야','탐앤탐스']
    gender_group.plot.bar(subplots = True, color =['red','green'], width =0.5 )
    plt.xlabel('커피 브랜드명')
